...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    """
      ...
      output (attn_value): [bs, seq_len, hidden_size]
    """
    ...
    # calculate attention_scores: 2 of [bs, num_attention_heads, seq_len, attention_head_size] 
    # -> [bs, num_attention_heads, seq_len, seq_len, attention_head_size]
    # -> [bs, num_attention_heads,seq_len,seq_len]
    attention_scores = torch.matmul(query, key.transpose(-1, -2))
    # attention_scores = torch.sum(query.unsqueeze(3) * key.unsqueeze(2),dim=-1)
    ...
    # [bs, num_attention_heads,seq_len,seq_len,1] * [bs, num_attention_heads, seq_len, 1, attention_head_size]
    # -> [bs, num_attention_heads,seq_len,seq_len,attention_head_size]
    # -> [bs, num_attention_heads,seq_len,attention_head_size]
    # attn_value = torch.sum(attention_probs.unsqueeze(-1) * value.unsqueeze(3),dim=-2)
    attn_value = torch.matmul(attention_probs, value)

    # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    bz,num_attention_heads,seq_len,attention_head_size = attn_value.size()
    attn_value = attn_value.permute(0,2,1,3).reshape(bz,seq_len,num_attention_heads*attention_head_size)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    """
    input: the input of BertSelfAttention
    output: the input that requires the sublayer to transform (output of BertSelfAttention)
    dense_layer, dropput: the sublayer
    ln_layer: layer norm that takes input+sublayer(output)
    """
    # # Drop-out of output
    # result = dropout(output)
    # Add input and output, forward to dense layer
    result = dense_layer(output)
    # Dropout,
    result = dropout(result)
    # Normalization 
    result = ln_layer(input+result)
    # Dropout, and return
    return result

  def forward(self, hidden_states, attention_mask):
    """
    hidden_states: either from the embedding layer (first bert layer) or from the previous bert layer - [batch_size, seq_len, hidden_size]
    ...
    attn_value = self.self_attention(hidden_states,attention_mask) # [bs, seq_len, hidden_size]

    # attention add-norm layer
    normed_attn_value = self.add_norm(input=hidden_states,output=attn_value,dense_layer=self.attention_dense,
                        dropout=self.attention_dropout,ln_layer=self.attention_layer_norm)

    # feed forward
    ff_output = self.interm_dense(normed_attn_value)
    ff_output = self.interm_af(ff_output)

    # another add-norm layer
    bert_output = self.out_dense(ff_output)
    bert_output = self.out_dropout(bert_output)
    bert_output = self.out_layer_norm(bert_output+normed_attn_value)
